...
step_iam() {
  ...
  aws iam delete-role-policy --role-name "$ROLE_NAME" --policy-name "$POLICY_NAME" 2>/dev/null \
    && echo "  deleted inline policy" || echo "  (no inline policy)"
  aws iam delete-role --role-name "$ROLE_NAME" 2>/dev/null \
    && green "  deleted role $ROLE_NAME" || gone
}
...
main() {
  case "${1:-}" in
    api|lambda|alarms|dynamodb|sns|iam|metrics|local) "step_${1}" ;;
    all)
      usage
      echo
      warn "This deletes the resources listed above, including the DynamoDB table"
      warn "and everything in it (seeded world, incidents, eval run history)."
      read -r -p "Type the table name '$TABLE_NAME' to confirm: " answer
      [ "$answer" = "$TABLE_NAME" ] || { echo "Aborted."; exit 1; }
      echo
      step_api; step_lambda; step_alarms; step_dynamodb; step_sns; step_iam; step_local
      echo
      step_metrics
